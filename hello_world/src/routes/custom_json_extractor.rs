use axum::{
    async_trait, body::HttpBody, extract::{FromRequest, RequestParts}, http::StatusCode, BoxError, Json
};
    ...
    B::Error: Into<BoxError>,
{
    ...
    async fn from_request(request: &mut RequestParts<B>) -> Result<Self, Self::Rejection> {
        let Json(user) = request.extract::<Json<RequestUser>>().await.map_err(|error|{
